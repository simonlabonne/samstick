...
export default function LevelPage({ title, apiUrl, optionA, optionB }: LevelProps) {
    ...
    useEffect(() => {
        fetch(apiUrl)
            ...
            .then((data) =>
                setItems(
                    data.map((d: { id: number; skaterFullName?: string; name?: string; shootsCatches?: string; answer?: string }) => ({
                        id: d.id,
                        label: d.skaterFullName ?? d.name ?? "Unknown",
                        correctAnswer: d.shootsCatches ?? d.answer ?? "",
                    }))
                )
            );
    }, [apiUrl]);
                ...
                {items.map((item) => (
                    ...
                    >
                        ...
                        <div className="flex justify-around">
                            <button
                                className="bg-burgundy transition hover:bg-burgundy-dark text-white px-4 py-2 rounded cursor-pointer"
                            ...
                            <button
                                className="bg-steel-blue transition hover:bg-steel-blue-dark text-white px-4 py-2 rounded cursor-pointer"
